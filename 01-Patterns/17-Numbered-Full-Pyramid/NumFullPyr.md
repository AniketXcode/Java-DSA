...
A simple Java program to print a **full number pyramid pattern** using nested loops. This is part of a Pattern Printing series under Data Structures & Algorithms practice.
...
1. A full number pyramid combines the **centering logic of a solid pyramid** with a **rise-and-fall numeric sequence** in each row — the numbers increase up to the row number and then decrease back down, forming a peak in the middle of each line.
2. For row `row`:
...
5. This continues until all `n` rows are printed, forming the complete full number pyramid.
